...
sys.path.extend('./')
sys.path.extend('../')
# from src.unet_lgm import UNet_lgm
...
def fov2focal_torch(fov, pixels):
...
class SingleImageSongUNetPredictor(nn.Module):
    def __init__(self, cfg, out_channels, bias, scale):
        ...
        self.out_channels = out_channels # [1-opacity 3-scale 4-rotation 3-feature 9-sh]
        self.cfg = cfg
        ...
        # print('model base dim: ', cfg['model']['base_dim'])
        # print('in_channels: ', in_channels)
        # print('num_blocks: ', cfg['model']['num_blocks'])
        # print('attn_resolutions: ', cfg['model']['attention_resolutions'])

        self.encoder = SongUNet(cfg['model']['base_dim'], 
                                in_channels, 
                                sum(out_channels),
                                # model_channels=cfg['model']['base_dim'], # new
                                num_blocks=cfg['model']['num_blocks'], # 4
                                emb_dim_in=emb_dim_in, # 0
                                channel_mult_noise=0,
                                attn_resolutions=cfg['model']['attention_resolutions']) # 16
...
class GaussianSplatPredictor_gtunet(nn.Module):
    def __init__(self, cfg):
        super(GaussianSplatPredictor_gtunet, self).__init__()
        self.cfg = cfg
        ...
        if cfg['model']['network_without_offset']: # here
        ...
        if cfg['model']['network_with_offset'] and cfg['model']['network_with_uncertainty']:
            split_dimensions, scale_inits, bias_inits = self.get_splits_and_inits_unc(True, cfg)
            self.network_with_offset = networkCallBack(cfg, 
                                        cfg['model']['name'],
                                        split_dimensions,
                                        scale = scale_inits,
    ...
    def init_ray_dirs(self):
        x = torch.linspace(-self.cfg['model']['training_resolution'] // 2 + 0.5, 
                            self.cfg['model']['training_resolution'] // 2 - 0.5, 
                            self.cfg['model']['training_resolution']) 
        y = torch.linspace( self.cfg['model']['training_resolution'] // 2 - 0.5, 
                           -self.cfg['model']['training_resolution'] // 2 + 0.5, 
                            self.cfg['model']['training_resolution'])
        ...
        #     or self.cfg.data.category == "objaverse" or self.cfg.data.category == "single":
        if True:
            ray_dirs[:, :2, ...] /= fov2focal(self.cfg['model']['fov'] * np.pi / 180, 
                                              self.cfg['model']['training_resolution'])
    ...
    def get_splits_and_inits(self, with_offset, cfg):
        ...
        if with_offset: # here
            ...
            # scale_inits = scale_inits + [cfg['model']['depth_scale'], # 1.0
            scale_inits = scale_inits + [ # 1.0
                           cfg['model']['xyz_scale'], # 0.000001
                           cfg['model']['opacity_scale'], # 0.001
                           cfg['model']['scale_scale'], # 0.001
                           1.0,
                           5.0]
            # bias_inits = [cfg['model']['depth_bias'], # 0.0
            bias_inits = [ # 0.0
                          cfg['model']['xyz_bias'], # 0.0
                          cfg['model']['opacity_bias'], # -3.0
                          np.log(cfg['model']['scale_bias']), # log(0.02)
                          0.0,
                          0.0]
        else: # here, no xyz_scale & xyz_bias
    ...
    def get_splits_and_inits_unc(self, with_offset, cfg):
        # Gets channel split dimensions and last layer initialisation
        split_dimensions = []
        scale_inits = []
        bias_inits = []
        ...
        if with_offset: # here
            split_dimensions = split_dimensions + [1, 3, 1, 3, 1, 3, 4, 3]
            scale_inits = scale_inits + [cfg['model']['depth_scale'], # 1.0 for unc
                           cfg['model']['xyz_scale'], # 0.000001 for offset2
                           ...
                           1.0,
                           5.0]
            bias_inits = [cfg['model']['depth_bias'], # 0.0 for unc
                          cfg['model']['xyz_bias'], # 0.0 for offset2
    ...
    def get_pos_from_network_output(self, depth_network, offset, focals_pixels, const_offset=None):
        ...
        # if self.cfg.data.category != "cars" and self.cfg.data.category != "chairs" and self.cfg.data.category != "single":
        if False:
        ...
        if const_offset is not None:
            # depth = self.depth_act(depth_network) * (self.cfg['dataset_params']['z_far'] - self.cfg['dataset_params']['z_near']) + self.cfg['dataset_params']['z_near'] + const_offset
            depth = depth_network + const_offset
        else: # here
            # depth = self.depth_act(depth_network) * (self.cfg['dataset_params']['z_far'] - self.cfg['dataset_params']['z_near']) + self.cfg['dataset_params']['z_near']
            depth = depth_network
    ...
    def forward(self, x, 
        ...
        # CO3D cameras are in NDC coordinates: see bottom of the page here. https://pytorch3d.org/docs/cameras.
        # if self.cfg.data.category == "cars" or self.cfg.data.category == "chairs" or self.cfg.data.category == "single":
        if True:
        ...
        if self.cfg['model']['origin_distances']: # false
            const_offset = x[:, 3:, ...]
            x = x[:, :3, ...]
        else:
            const_offset = None
        ...
        if self.cfg['model']['network_with_offset']: # not here
            ...
            # get 3D position by back-projection
            pos = self.get_pos_from_network_output(unet_depth, offset, focals_pixels, const_offset=const_offset)
            ...
            # get 3D position by back-projection
            pos = self.get_pos_from_network_output(unet_depth, 0.0, focals_pixels, const_offset=const_offset)

        if self.cfg['model']['isotropic']:  # false
        ...
        if squre_clip < 10.0:
            pos[:, :, 0].clamp_(-squre_clip, squre_clip)
            pos[:, :, 1].clamp_(-squre_clip, squre_clip)
        ...
        out_dict = {
            ...
            "unet_depth": self.flatten_vector(unet_depth) # bchw -> bnc
                }
